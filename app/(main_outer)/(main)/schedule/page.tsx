...
export default async function ScheduleHome() {
  const payload = await getPayload({config:configPromise});
  const generalteamevents = (await payload.find({collection:"event",pagination:false,depth:1,where:{eventType:{equals:"general"},forAll:{equals:"teams"}}})).docs
